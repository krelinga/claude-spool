...
Guidance for the Go service. The repo root has its own CLAUDE.md covering the
monorepo as a whole; this file applies to everything under `backend/`.

Design: [`docs/design/backend-design-doc.md`](docs/design/backend-design-doc.md).
Section references like §3.2 throughout this file and the code point at it.
...
All Go commands run from `backend/`, which is where `go.mod` lives.
